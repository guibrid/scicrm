...
class CatalogueHelpers
{
  ...
  public function getProductsToDisplay($array, $qualification = null)
  {
    ...
    foreach ($array as $key => $value) {
      ...
      if ($qualification == 'M') {
        ...
        if (!isset($listeProduct[0]['Produits'])) {
          $listeProduct[] = ['Marque' => 'MDD'];
        }
  ...
  public function orderProduct($listQualificationP, $listQualificationM, $listQualificationA)
  {
    ...
    $listProductOrdered = array_merge(
      $this->getProductsToDisplay($listQualificationM, 'M'),
  ...
  {
    if (isset($productDetails->photos[0]->url) && $productDetails->photos[0]->active == 2 && $productDetails->photos[0]->url <> '-') {
      if (strlen($productDetails->photos[0]->url) < 25) {
        ...
        $photo = 'https://hecate-export.com/photos/' . $productDetails->photos[0]->url;
      } else {
    ...
    if ($type == 'catalogue') {
      $productRow = [
        ...
        '',
        $photo,
        $this->formatNew($productDetails->new),
        $productDetails->duree_vie,
        $this->formatDLV($productDetails->dlv),
        $productDetails->title,
        $this->formatBrand($productDetails->Brands['title']),
        $productDetails->pieceartk,
        $productDetails->pcb,
        ...
        '',
        '',
        $productDetails->poids,
        $productDetails->volume,
        (int)$productDetails->couche_palette,
        (int)$productDetails->colis_palette,
        (string)$productDetails->douanier,
        $productDetails->qualification,
        $productDetails->gencod,
        ''
      ];
    } else if ($type == 'boncommande') {
      if ($productDetails->remplacement_product == 'Carrefour') {
        $productDetails->remplacement_product = '';
      }
      $productRow = [
        $productDetails->code,
        $productDetails->remplacement_product,
        ...
        (string)$productDetails->douanier,
        '',
        $productDetails->dangereux,
        $productDetails->Origins['title']
      ];
    }
